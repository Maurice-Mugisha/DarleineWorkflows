...
if(isset($_GET['specific_privilege'])):


	$privilege_id = isset($_GET['privilege_id']) ? $fieldValidator->validate_field($_GET['privilege_id']) : "";

	$roleprivilegemap_array = array();
	$privilege_array = array();
	$role_array = array();

	$query = $queryExecutor->execute($selectionObj->select_privilege($privilege_id));
	...
	while($row = mysqli_fetch_array($query)):
		...
		$privilege_array[$key]['name'] = $row_col['name'];

		$dependent_query = $queryExecutor->execute($selectionObj->select_privilege_description($privilege_id));
		$row_col = mysqli_fetch_array($dependent_query);
		$privilege_array[$key]['description'] = $row_col['description'];


		$secondary_query = $queryExecutor->execute($selectionObj->select_roleprivilegemap_by_privilege($privilege_id));
...
<table cellspacing = "1" cellpadding = "0" align = "center" width = "100%" id = "roleprivilegemap">
 <tr>
 <td colspan = "3">&nbsp;&nbsp;&nbsp;&nbsp;</td>
 <td colspan = "2"><a href="target_print.php?roleprivilegemap&&privilege_id=<?= $privilege_id ?>" target="_blank" style="color:#00FF00;">Print PDF</a></td></tr>
 <tr class = "top_row">
 <td>&nbsp;&nbsp;#&nbsp;&nbsp;</td>
 <td>&nbsp;Role id&nbsp;</td>
 <td>&nbsp;Name&nbsp;</td>
